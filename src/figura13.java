/*
 * Copyright (c),JfraGar
 * (javaGenius) jfchanatasig1405@gmail.com
 * version 1.0
 */
public class figura13 {
    /**
     * Imprime la figura 13 con un tamaño específico.
     *
     * @param tamano Tamaño de la figura 13 a imprimir.
     */
    public void imprimirFigura13(int tamano){
        // Bucle externo para iterar desde 1 hasta el tamaño proporcionado
        for (int i = 1; i <= tamano; i++) {
            // Bucle interno para imprimir números del 1 al i
            for (int j = 1; j <= i; j++) {
                System.out.print(j);
            }
            // Salto de línea después de imprimir los números
            System.out.println();
        }
    }
}